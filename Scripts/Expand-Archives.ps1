...
do {
    ...
    if ($sevenZip) {
        $has7z = $true
        if ($sevenZip.Source) { $sevenZipPath = $sevenZip.Source }
    }

    $recurseChoice = Read-Host "是否递归子目录？(Y/N，默认N)"
    $recurse = ($recurseChoice.Trim().ToLower() -eq "y")

    $gciParams = @{ LiteralPath = $targetPath; File = $true }
    if ($recurse) { $gciParams.Recurse = $true }

    if ($has7z) {
        $archives = Get-ChildItem @gciParams | Where-Object {
        ...
        $compressedButUnsupported = Get-ChildItem @gciParams | Where-Object {
        ...
        foreach ($arc in $archives) {
            ...
            try {
                if ($has7z -and ($arc.Extension -in ".7z", ".rar")) {
                    & $sevenZipPath x $arc.FullName ("-o{0}" -f $dest) -y | Out-Null
