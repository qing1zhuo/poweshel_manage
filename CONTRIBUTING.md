...
本项目区分 **“管理工具核心 (src/)”** 和 **“业务脚本 (scripts/)”** 两类开发任务。

### 1.1 修改/新增业务脚本 (`scripts/` 目录下)
**严禁直接手动修改 `scripts/` 下的文件！** 必须通过管理工具进行操作，以触发自动备份机制。

1. **运行管理工具**：在终端执行 `python main.py` 并等待 GUI 启动。
2. **选择操作**：
   - **新增脚本**：点击侧边栏的“✨ 新增脚本”按钮，填写名称与描述，工具会自动生成 `.ps1` 文件并调用记事本打开。
   - **修改脚本**：在脚本列表中找到目标条目并点击右侧的“✏️ 修改”按钮。管理器会先把当前版本备份到 `backups/`，然后打开记事本供编辑。
...
### 1.2 修改管理工具核心 (`src/` 目录下)
由于管理工具已模块化，对其修改需遵循以下规范：

1. **功能归类**：
    - 界面修改：`src/ui/app.py` 或 `src/ui/dialogs.py`。
    - 逻辑修改：`src/core/script_logic.py`。
    - 配置修改：`src/config/settings.py`。
2. **本地测试**：修改后运行 `python main.py`，验证界面加载正常、脚本列表显示、运行/编辑/回滚等功能均可用。
3. **更新元数据逻辑**：若改动涉及 `data/metadata.json` 读写，请检查兼容性与 JSON 格式。
