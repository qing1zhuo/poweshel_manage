...
## [2026-04-10] - 架构大考：模块化拆分与工程化标准 (v2.1)

### 🏗️ 架构重构 (工程化升级)
- **模块化拆分**：将单体 `ScriptManager.py` 拆分为分层架构：
    - `src/core/`：核心业务逻辑（备份、元数据管理、文件操作）。
    - `src/ui/`：界面层（主窗口、交互对话框）。
    - `src/config/`：全局配置（路径管理、主题初始化）。
- **标准目录结构**：
    - 源码移至 `src/`。
    - 元数据移至 `data/metadata.json`。
    - 备份目录重命名为 `backups/`。
    - 业务脚本目录统一为 `scripts/`。
- **零配置启动 (Portable)**：引入 `BASE_DIR` 动态路径计算，彻底移除所有硬编码绝对路径，项目支持在任意目录下即插即用。
- **全新入口**：启用 `main.py` 作为程序唯一启动点，根目录保持精简。

### ✨ 功能增强
- **自动化备份闭环**：在编辑、回滚操作前强制触发自动备份，确保脚本安全。
- **元数据自动维护**：新增脚本或修改脚本时，版本号与描述信息实现毫秒级自动同步更新。
- **现代化对话框**：重写了“新增脚本”与“版本回滚”界面，支持居中弹出、按时间排序备份列表等。
- **健壮性修复**：全面采用 `utf-8-sig` 编码读写 JSON，完美解决因 UTF-8 BOM 导致的程序启动崩溃问题。

---

## [2026-04-10] - 新增脚本：Merge-ImagesToPdf.ps1

### ✨ 新增功能
- **Merge-ImagesToPdf.ps1**：将指定文件夹下的所有图片按名称字典序合成为一个 PDF 文件。
- **智能命名**：自动提取首张图片的名字作为生成的 PDF 文件名。
- **排序规则**：严格遵循文件名字典序排序，确保合成顺序符合预期。
- **格式支持**：支持 JPG, JPEG, PNG, BMP, WEBP 等主流图片格式。
- **交互规范**：遵循项目 `CONTRIBUTING.md` 规范，包含功能说明、环境检测、循环执行及默认确认逻辑。

### 🛠️ 故障修复
- **修复图片扫描逻辑**：解决了由于 `Get-ChildItem -Include` 机制导致的在某些环境下无法正确匹配目录内图片的问题。现改为基于扩展名过滤的健壮方案。
- **版本**：Merge-ImagesToPdf.ps1 升级至 v2。

---

## [2026-03-03] - 架构跨越：Python 全新交互时代 (v2.0)

### 🚀 技术栈重构
- **核心迁移**：放弃原有的 PowerShell WPF/CLI 架构，全面迁移至 **Python 3.12 + CustomTkinter**。
- **纯本地 UI**：不依赖浏览器环境，实现高性能、响应式的现代桌面交互。
- **静默启动**：通过 `pythonw.exe` 调用方案，彻底解决启动时闪烁黑色控制台窗口的顽疾。

### 💻 革命性控制台交互
- **内嵌式交互终端**：在主界面直接集成实时输出窗口，告别弹出式控制台。
- **提示词捕获修复**：通过注入 `Read-Host` 包装函数，完美解决了 PowerShell 在流重定向模式下无法显示“请输入路径”等提示词的底层限制。
- **零延迟渲染**：采用逐字符读取逻辑与无缓冲模式，确保脚本输出与用户输入同步显示，支持复杂的交互式脚本。
- **视觉区分**：用户输入以**绿色**高亮显示，与脚本输出清晰区分。

### 🎨 UI/UX 深度优化
- **现代视觉风格**：引入圆角卡片布局、平滑阴影及 Consolas 编程字体。
- **原生主题支持**：集成系统级深色/浅色模式自动切换功能。
- **操作链路优化**：点击“运行”后自动聚焦输入框；支持回车键一键发送指令。

### 🧹 项目清理与文档
- **架构精简**：移除了已过时的 `ScriptManager.ps1` 和 `ScriptManagerGUI.ps1`。
- **文档同步**：更新了 `README.md`，新增了 Python 环境依赖说明及桌面快捷方式创建指南。
...
- **分支推送兼容性**：修复了当本地分支名为 `master` 时无法推送到远程 `main` 的问题。现在采用 `git push origin HEAD:main` 策略，确保当前分支（无论名称如何）都能正确同步至远程主分支。

### ✨ 脚本功能增强 (Push-ToGitHub.ps1)
- **自定义提交备注**：在提交代码前新增询问环节，支持用户选择手动输入备注或使用系统默认备注。
- **元数据同步**：版本号提升至 v3，更新了功能描述。
...
### 🎨 UI 与交互优化
- **控制台窗口彻底关闭**：优化了 `ScriptManagerGUI.ps1` 的启动机制，通过自我重启后台进程并退出原始进程的方式，实现了黑色控制台窗口的彻底关闭（而非仅仅隐藏），提供更极致的图形化体验。
